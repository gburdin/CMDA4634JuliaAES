include("Galois_Serial.jl")

global const sub_box = [

]

global const inv_sub_box = [

]

global const mix_col = [
0x02, 0x03, 0x01, 0x01,
0x01, 0x02, 0x03, 0x01,
0x01, 0x01, 0x02, 0x03,
0x03, 0x01, 0x01, 0x02
]

global const inv_mix_col = [
0x0e, 0x0b, 0x0d, 0x09,
0x09, 0x0e, 0x0b, 0x0d,
0x0d, 0x09, 0x0e, 0x0b,
0x0b, 0x0d, 0x09, 0x0e
]

global const num_blocks = 4
global const block_rows = 4

function AESCipher(inBytes::Array{UInt8, 1}, w::Array{UInt8, 1}, num_rounds::Int) # <--------- 4
	@assert(block_rows == num_blocks)
	@assert(length(inBytes) == (block_rows * num_blocks))
	@assert(length(w) == (block_rows * num_blocks * (num_rounds + 1)))

	state = copy(inBytes)
	for j in 1:16
		state[j] = xor(state[j], w[j])
	end

	cache = Array{UInt8}(undef, num_blocks)

	for round=1:(num_rounds-1)
		SubBytes(state)
		ShiftRows(state)
		MixColumns(state, cache)
		range = (round * 16 + 1):(round  * 16 + 16)
		for j in 1:16
			state[j] = xor(state[j], w[range[j]])
			# $state[j] = xor($state[j], $w[round * 16 + j])
		end
	end

	SubBytes(state)
	ShiftRows(state)
	# AddRoundKey(state, w[(num_rounds * num_blocks * block_rows + 1):((num_rounds + 1) * num_blocks * block_rows)])
	range = (num_rounds * 16 + 1):(num_rounds * 16 + 16)
	for j in 1:16
		state[j] = xor(state[j], w[range[j]])
	end

	return state
end

# Encrypts the given plaintext and key
function AESEncrypt(plain::String, key::String) # <--------- 1 
	bytes2hex(AESEncrypt(hex2bytes(plain), hex2bytes(key)))
end

# Decrypts the given ciphertext and key
function AESDecrypt(cipher::String, key::String)
	bytes2hex(AESDecrypt(hex2bytes(cipher), hex2bytes(key)))
end

# Encrypts the given plaintext "block" using the given key
function AESEncrypt(plain::Array{UInt8, 1}, key::Array{UInt8, 1}) # <--------- 2
	(w, num_rounds) = AEScrypt(plain, key)
	return AESCipher(plain, w, num_rounds)
end


function AESEncrypt_modes(plain::Array{UInt8, 1}, w::Array{UInt8, 1}, num_rounds::Int)
	return AESCipher(plain, w, num_rounds)
end

# Decrypts the given ciphertext "block" and key
function AESDecrypt(cipher::Array{UInt8, 1}, key::Array{UInt8, 1})
	(w, num_rounds) = AEScrypt(cipher, key)
	return AESInvCipher(cipher, w, num_rounds)
end

function AESDecrypt_modes(cipher::Array{UInt8, 1}, w::Array{UInt8, 1}, num_rounds::Int)
	return AESInvCipher(cipher, w, num_rounds)
end


# General function for checking the parameters of the encryption/decryption
# functions and expanding the key.
function AEScrypt(input::Array{UInt8, 1}, key::Array{UInt8, 1}) # <--------- 3
	if length(input) != (block_rows * num_blocks)
		error("input not 16 byte block")
	end
	(num_keys, num_rounds) = AESParameters(key) # 
	w = KeyExpansion(key, num_keys, num_rounds)
	return (w, num_rounds)
end

# Return a tuple (num_keys, num_rounds) where num_keys is the number of key blocks
# and num_rounds is the number of rounds for the key size.
function AESParameters(key::Array{UInt8, 1})

    num_keys_lookup = [4, 6, 8]
    num_blocks_lookup = [10, 12, 14]

	if mod(length(key), block_rows) != 0
		error("the key length must be a multiple of four!")
	end

	num_keys = div(length(key), block_rows)
	i = first(indexin([num_keys], num_keys_lookup))

	if i == 0
		error("key length is non-standard!")
	end

	num_rounds = num_blocks_lookup[i]
	return (num_keys, num_rounds)
end

function KeyExpansion(key::Array{UInt8, 1}, num_keys::Int, num_rounds::Int)
	@assert(length(key) == (block_rows * num_keys))

	w = Array{UInt8}(undef, block_rows * num_blocks * (num_rounds + 1))
	w[1:(block_rows * num_keys)] = copy(key)
	i = num_keys

	while i < (num_blocks * (num_rounds + 1))
		temp = w[((i - 1) * block_rows + 1):(i * block_rows)]
		if mod(i, num_keys) == 0
			temp = xor.(SubWord(RotWord(temp)) , Rcon(div(i, num_keys)))
		elseif (num_keys > 6) && (mod(i, num_keys) == num_blocks)
			temp = SubWord(temp)
		end
		w[(i * block_rows + 1):((i + 1) * block_rows)] = xor.(w[((i - num_keys) * block_rows + 1):((i - num_keys + 1) * block_rows)] , temp)
		i += 1
	end

	return w
end

function SubWord(w::Array{UInt8, 1})
	@assert(length(w) == block_rows)
	map!(x -> sub_box[Int(x) + 1], w, w)
	return w
end

function RotWord(w::Array{UInt8, 1})
	@assert(length(w) == block_rows)
	permute!(w, [2, 3, 4, 1])
end

function Rcon(i::Int)
	@assert(i > 0)
	x = 0x01
	for j=1:(i-1)
		x = gmul(x, 0x02)
	end
	return [x, 0x00, 0x00, 0x00]
end

function AESInvCipher(inBytes::Array{UInt8, 1}, w::Array{UInt8, 1}, num_rounds::Int)
	@assert(block_rows == num_blocks)
	@assert(length(inBytes) == (block_rows * num_blocks))
	@assert(length(w) == (block_rows * num_blocks * (num_rounds + 1)))

	state = copy(inBytes)
	# AddRoundKey(state, w[(num_rounds * num_blocks * block_rows + 1):((num_rounds + 1) * num_blocks * block_rows)])
	range = (num_rounds * 16 + 1):(num_rounds * 16 + 16)
	for j in 1:16
		state[j] = xor(state[j], w[range[j]])
	end

	cache = Array{UInt8}(undef, num_blocks)

	for round=(num_rounds-1):-1:1
		InvShiftRows(state)
		InvSubBytes(state)
		# AddRoundKey(state, w[(round * num_blocks * block_rows + 1):((round + 1) * num_blocks * block_rows)])
		range = (round * 16 + 1):(round * 16 + 16)
		for j in 1:16
			state[j] = xor(state[j], w[range[j]])
		end
		InvMixColumns(state, cache)
	end

	InvShiftRows(state)
	InvSubBytes(state)
	# AddRoundKey(state, w[1:(num_blocks * block_rows)])
	for j in 1:16
		state[j] = xor(state[j], w[j])
	end

	return state
end

# Returns one-based column indices of the given column (one-based).
function columnIndices(column::Int)
	return column:num_blocks:num_blocks*num_blocks
end

# Returns row indices of the given row (one-based).
function rowIndices(row::Int)
	return ((row - 1) * num_blocks + 1):(row * num_blocks)
end

function SubBytes(a::Array{UInt8, 1})
	SubBytesGen(a, false)
end

function InvSubBytes(a::Array{UInt8, 1})
	SubBytesGen(a, true)
end

function SubBytesGen(a::Array{UInt8, 1}, inv::Bool)
	box = inv ? inv_sub_box : sub_box
	f = (x) -> box[Int(x) + 1]
	map!(f, a, a)
	return a
end

function ShiftRows(a::Array{UInt8, 1})
	ShiftRowsGen(a, false)
end

function InvShiftRows(a::Array{UInt8, 1})
	ShiftRowsGen(a, true)
end

function ShiftRowsGen(a::Array{UInt8, 1}, inv::Bool)
	# permute columns in _last three_ rows
	# note that a column is actually a row in memory
	if inv
		tmp1 = a[14]
		a[14] = a[10]
		a[10] = a[6]
		a[6] = a[2]
		a[2] = tmp1

		tmp1 = a[3]
		a[3] = a[11]
		a[11] = tmp1
		tmp1 = a[7]
		a[7] = a[15]
		a[15] = tmp1

		tmp1 = a[4]
		a[4] = a[8]
		a[8] = a[12]
		a[12] = a[16]
		a[16] = tmp1
	else
		tmp1 = a[2]
		a[2] = a[6]
		a[6] = a[10]
		a[10] = a[14]
		a[14] = tmp1

		tmp1 = a[3]
		a[3] = a[11]
		a[11] = tmp1
		tmp1 = a[7]
		a[7] = a[15]
		a[15] = tmp1

		tmp1 = a[16]
		a[16] = a[12]
		a[12] = a[8]
		a[8] = a[4]
		a[4] = tmp1
	end
	return a
end

function MixColumns(a::Array{UInt8, 1}, cache)
	MixColumnsGen(a, false, cache)
end

function InvMixColumns(a::Array{UInt8, 1}, cache)
	MixColumnsGen(a, true, cache)
end

function MixColumnsGen(a::Array{UInt8, 1}, inv::Bool, cache)
	# note that columns are actually rows in memory
	matrix = inv ? inv_mix_col : mix_col
	# for c=1:num_blocks
	# 	indices = rowIndices(c)
	# 	for i = 1 : num_blocks
	# 		cache[i] = a[indices[i]]
	# 	end
	# 	# @assert(length(cache) == num_blocks)
	# 	# Matrix multiplication with Galois field operations
	# 	for r=1:num_blocks
	# 		mi = matrix[rowIndices(r)]
	# 		a[indices[r]] = gadd(map(gmul, cache, mi))
	# 	end
	# end
	for j in 1:4
		cache[1] = gmul(a[4j-3], matrix[1]) ⊻ gmul(a[4j-2], matrix[2]) ⊻ gmul(a[4j-1], matrix[3]) ⊻ gmul(a[4j], matrix[4])
		cache[2] = gmul(a[4j-3], matrix[5]) ⊻ gmul(a[4j-2], matrix[6]) ⊻ gmul(a[4j-1], matrix[7]) ⊻ gmul(a[4j], matrix[8])
		cache[3] = gmul(a[4j-3], matrix[9]) ⊻ gmul(a[4j-2], matrix[10]) ⊻ gmul(a[4j-1], matrix[11]) ⊻ gmul(a[4j], matrix[12])
		cache[4] = gmul(a[4j-3], matrix[13]) ⊻ gmul(a[4j-2], matrix[14]) ⊻ gmul(a[4j-1], matrix[15]) ⊻ gmul(a[4j], matrix[16])
		a[4j-3] = cache[1]
		a[4j-2] = cache[2]
		a[4j-1] = cache[3]
		a[4j  ] = cache[4]
	end
	return a
end

function AddRoundKey(s::Array{UInt8, 1}, w::Array{UInt8, 1})
	# @assert(length(w) == (block_rows * num_blocks) && (block_rows == num_blocks))
	# return map!(gadd, s, s, w)
	for i = 1 : 16
		s[i] = xor(s[i], w[i])
	end
end